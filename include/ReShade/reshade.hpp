...
namespace reshade
{
	...
	inline bool register_addon(HMODULE module)
	{
		...
		// Check that the ReShade module was built with Dear imgui2 support and supports the used version
		const auto imgui2_func = reinterpret_cast<const imgui2_function_table *(*)(uint32_t)>(
			GetProcAddress(reshade_module, "ReShadeGetimgui2FunctionTable"));
		if (imgui2_func == nullptr || !(imgui2_function_table_instance() = imgui2_func(imgui2_VERSION_NUM)))
			return false;
#endif
	...
	/// <para>The callback function is then called when the overlay is visible and allows adding Dear imgui2 widgets for user interaction.</para>
	/// </summary>
