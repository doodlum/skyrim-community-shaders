...
#	elif defined(BLEND)
	...
	float3 ppColor = 0.0;
	{
        ...
        ppColor = linearColor;	// ppColor = max(0, linearColor);
	}

	// HDR tonemapping

    float3 hdrColor = Color::GammaToLinearSafe(ppColor);

	float peakWhite = lerp(1.0, HDRData.y / HDRData.z, HDRData.x);
    ...
    // hdrColor = ExtendGamut(hdrColor, 1.0);	// test if negatives are clamped

    hdrColor = Color::LinearToGammaSafe(hdrColor);

#		if defined(FADE)
	hdrColor = lerp(hdrColor, Fade.xyz, Fade.w);
#		endif

    hdrColor = FrameBuffer::ToSRGBColorSafe(hdrColor);

    hdrColor = Color::GammaToLinearSafe(hdrColor);
    hdrColor /= peakWhite;
    hdrColor = Color::LinearToGammaSafe(hdrColor);
