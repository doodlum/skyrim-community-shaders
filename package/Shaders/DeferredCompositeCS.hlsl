...
[numthreads(32, 32, 1)] void MainCompositePass(uint3 globalId
	...
	float3 specularIrradiance = EnvTexture.SampleLevel(LinearSampler, R, 1).xyz;
	specularIrradiance = sRGB2Lin(specularIrradiance);

	float3 specularIrradiance2 = ReflectionTexture.SampleLevel(LinearSampler, R, 1).xyz;
	...
	float3 positionWScamera = positionMS.xyz;
	float3 positionWSreal = positionMS.xyz + CamPosAdjust[0].xyz;
	float3 positionWSvoxel = positionMS.xyz - voxel_radiance.GridCenter;

	float4 voxel = VoxelTexture.SampleLevel(LinearSampler, positionWSvoxel, 0);

	MainRW[globalId.xy] = half4(voxel.xyz, 1.0);
