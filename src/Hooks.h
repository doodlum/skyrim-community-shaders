 ...
 *         pointer (populated by the CreateVertexShader/CreatePixelShader capture hooks when the
 *         shader cache is dumping). Returns false if the shader was not captured. Safe to call
 *         concurrently after capture (read-only lookup). */
