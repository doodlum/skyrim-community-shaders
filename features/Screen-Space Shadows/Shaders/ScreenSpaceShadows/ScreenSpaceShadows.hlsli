...
float GetScreenSpaceShadow(float2 a_uv, float3 a_viewPosition, uint a_eyeIndex)
{
	float weight = 0;
	float shadow = 0;
	...
	for (uint i = 0; i < 8; i++) {
		float2 offset = BlurOffsets[i] * 0.0025;
		...
		float attenuation = 1.0 - saturate(abs(linearDepth - a_viewPosition.z) / (1.0 + a_viewPosition.z));
		if (attenuation > 0.0) {
